...
const handleCall = async () => {
  try {
    const res = await axios.post("http://localhost:5000/api/leads/call", {
      name: lead.name,
      phone: lead.phone,
      sessionId: lead._id, // or any session identifier you use
    });
    alert("Call initiated! ID: " + res.data.callId);
  } catch (error) {
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-1">
            ...
            <span
              className={`text-sm font-medium ${getTimelineColor(
                lead.timeline
              )}`}
            >
